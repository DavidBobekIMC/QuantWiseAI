...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item MACD (Moving Average Convergence Divergence): MACD is a trend-following momentum indicator that shows the relationship between 
			two moving averages of a so called security’s price.
			\item VOLD (Volume Delta): Volume Delta is a volume-based indicator that measures the flow of money in and out of a security.
...
\chapter{Experimentation}
	...
	\section{Data Science}
		...
		\subsection{Data Analysis}
			I have created a separate Juptyer Notebook ($volume_time_analysis.ipynb$) that contains an analysis of what is the busiest hour during the day and what is the busiest hour during the 
			...
			when London Stock Exchange opens and closes. The London Stock Exchange is the largest stock exchange in Europe and the 3rd largest in the world.
			...
			to see the differences. This plots tracks 400 hours worth of trading as the interval in between each of the points is 4 hours and there are 100 points in total.
			...
			This way we can clearly see the integer values of each of the hours and how the price is moving during these hours. This is a fairly interesting finding as we can see that sometimes the 
			...
			world which is the currency exchange market. This suddden change in the price can be explained by the "internal agreement" of the market to move uni directionally. In other cases we can
			...
			The bulls are not able to overpower the bears and the bears are not able to overpower the bulls and therefore the price is not moving at all. 
	
			In certain cases like $INESERT HEREEEE$ we can see that price is moving in a certain direction and then suddenly it changes its direction and starts moving in the opposite direction.
			...
			explained by the opening of the market and the sheer volume of trading that is happening during these hours is able to change the direction of the market.
